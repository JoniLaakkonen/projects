...
enum SlickPageChangedReason { timed, manual, controller }

enum CenterPageEnlargeStrategy { scale, height, zoom }

class SlickOptions {
  /// Set slick height and overrides any existing [aspectRatio].
  final double? height;
  ...
  ///
  /// Defaults to 0.8, which means each page fills 80% of the slick.
  final double viewportFraction;

  /// The initial page to show when first creating the [SlickSlider].
  ///
  /// Defaults to 0.
  final int initialPage;

  ///Determines if slick should loop infinitely or be limited to item length.
  ///
  ///Defaults to true, i.e. infinite loop.
  final bool enableInfiniteScroll;

  ///Determines if slick should loop to the closest occurence of requested page.
  ///
  ///Defaults to true.
  ...
  /// creating a feeling of depth in the slick.
  ///
  /// Defaults to false.
  ...
  final Function(int index, SlickPageChangedReason reason)? onPageChanged;

  /// Called whenever the slick is scrolled
  final ValueChanged<double?>? onScrolled;

  /// How the slick should respond to user input.
  ///
  ...
  /// the slick, and will be resumed when user finish interacting.
  /// Default to `true`.
  ...
  /// decide the slick should go to the first item when it reach the last item or not.
  ...
  ///Generate new [SlickOptions] based on old ones.

  SlickOptions copyWith(
          {double? height,
          double? aspectRatio,
          ...
          Function(int index, SlickPageChangedReason reason)? onPageChanged,
